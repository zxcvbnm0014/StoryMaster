...
function load() {
    ...
    if (playButtons) {
        ...
        if (!added) {
            playButtons.platformList = [].concat(playButtons.platformList, {text: PreviewUnit, value: PkgPreview.unit});
        }
    }
    else {
        let b = addTagToPlayButtonsWith$2_1_1();
        if (!b) {
            Editor.warn(`[Story-Master]: 插件加载异常,可能是你的Creator版本过高导致[故事测试],[故事预览]快捷功能无法使用!`);
        }
    }
}
...
module.exports = {
    ...
    unload: function () {
        ...
        if (playButtons) {
            ...
            playButtons.platformList = playButtons.platformList.filter(x => x.value !== PkgPreview.unit);
            playButtons.platformList = playButtons.platformList.filter(x => x.value !== PkgPreview.story);
        } else {
