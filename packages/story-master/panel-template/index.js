...
const TemplateMsg = Editor.require(
    'packages://story-master/panel-template/msg.js'
);
...
Editor.Panel.extend({
    ...
    ready() {
        this.plugin = new window.Vue({
            ...
            created() {
                this.$root.$on(
                    TemplateMsg.OnCustomTemplateRightMenu,
                    this._onCustomTemplateRightMenu
                );
                (async () => {
                    console.log('created');
                    // 先读取profile里面的配置
                    Editor.Profile.load(
                        StoryMaster.GameCfg.profile,
                        function(error, profile) {
                            if (error) {
                                ...
                                this.templateTalkPrefab =
                  profile.data.templateTalkPrefab || template.StoryTalk;
                                ...
                                this.templateOptionPrefab =
                  profile.data.templateOptionPrefab || template.StoryOptions;
                                Editor.assetdb.queryUuidByUrl(
                                    this.templateOptionPrefab,
                                    function(error, uuid) {
                                        if (error) {
            ...
            methods: {
                _onCustomTemplateRightMenu(event, x, y, data) {
                    const RightMenu = Editor.require(
                        'packages://story-master/core/rightMenu.js'
                    );
                    let template = [
                        {
                            label: '删除',
                            click: () => {
                                this.onCustomTemplateMenu(data);
                            },
                        },
                    ];
                ...
                async setDefaultCfg() {
                    ...
                    if (Fs.existsSync(fullUrl1)) {
                        let ret = await Core.queryUuidByUrl(template.StoryTalk);
                    ...
                    if (Fs.existsSync(fullUrl2)) {
                        let ret = await Core.queryUuidByUrl(template.StoryOptions);
                ...
                onBtnClickAddTalkPrefab() {
                    ...
                    if (prefabUUID) {
                        // 最终选择的了这种方式
                        Core.callSceneScript('existsScriptInRoot', scriptName, function(
                            ret
                        ) {
                            if (ret) {
                                ...
                                Core.callSceneScript('addPrefabToCurrent', prefabUUID, function(
                                    error,
                                    uuid
                                ) {
                                    if (error) {
