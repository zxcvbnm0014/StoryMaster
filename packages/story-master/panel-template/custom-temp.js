...
module.exports = function () {
    Vue.component("custom-temp", {
        ...
        created() {
            if (this.data.prefab) {
                Editor.assetdb.queryUuidByUrl(this.data.prefab, function (error, uuid) {
                    if (error) {
                        console.log(error);
                    } else {
                        if (uuid) {
        ...
        methods: {
            onCustomTempMenu() {
                Editor.Ipc.sendToMain('story-master:onCustomTemplateMenu', event.x, event.y, this.data);
            },
            onChangeName() {
                this.isRename = false;
            ...
            onBtnAddPrefab() {
                if (this.prefabUUID) {
                    Core.callSceneScript("addPrefabToCurrent", this.prefabUUID, function (error, uuid) {
                        if (error) {
                            console.log(error)
                        } else {
                        }
            ...
            onChangePrefab() {
                ...
                Editor.assetdb.queryUrlByUuid(this.prefabUUID, function (error, url) {
                    if (error) {
                        console.log(error);
                    } else {
                        if (url) {
