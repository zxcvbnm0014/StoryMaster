...
module.exports = {
    ...
    _saveInitCfgUnit(cb) {
        Editor.Ipc.sendToPanel('story-master.piece', 'getCurrentEditorPiecePrefab', function (id) {
            if (id) {
    ...
    messages: {
        ...
        setPieceData(event, data) {
            ...
            if (data.OpenedPrefabID !== undefined) {// 打开的prefab
        ...
        onCustomTemplateMenu(event, x, y, data) {
            ...
            let template = [
                {
                    label: '删除', click() {
                        Editor.Ipc.sendToPanel('story-master.template', 'onCustomTemplateMenu', data);
                    }
                },
            ];
        ...
        onPieceItemMenu(event, x, y, data, options) {
            ...
            let template = [
                {
                    label: '插入模版片段(前边)', click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuInsertItemBefore', data);
                    }
                },
                {
                    label: '插入模版片段(后边)', click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuInsertItemAfter', data);
                    }
                },
                {type: 'separator'},
                {
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuItemUp', data);
                    }
                },
                {
                    label: '下移',
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuItemDown', data);
                    }
                },
                {
                    label: '下移到末尾',
                    enabled: options && options.down,
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuItemDownEnd', data);
                    }
                },
                {type: 'separator'},
                {
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuItemCut', data);
                    }
                },
                {
                    label: '复制',
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuCopyItem', data);
                    }
                },
                {
                    label: '粘贴',
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuPastItem', data);
                    }
                },
                {type: 'separator'},
                {
                    label: '删除', click() {
                        Editor.Ipc.sendToPanel('story-master.piece', 'onPieceMenuDel', data);
                    }
                },
                {type: 'separator'},
                {
                    label: "测试（需开启测试模式）", click() {
        ...
        onPlotItemMenu(event, x, y, data, options, bCut, bPast) {
            ...
            let template = [
                ...
                {type: 'separator'},
                {
                    label: '上移',
                    click() {
                        Editor.Ipc.sendToPanel('story-master.plot', 'onPlotMenuItemUp', data);
                    }
                },
                {
                    label: '下移',
                    click() {
                        Editor.Ipc.sendToPanel('story-master.plot', 'onPlotMenuItemDown', data);
                    }
                },
                {type: 'separator'},
                // {
                ...
                // },
                {
                    ...
                    enabled: options.cut,
                    click() {
                        Editor.Ipc.sendToPanel('story-master.plot', 'onPlotMenuItemCut', data);
                    }
                },
                {
                    label: '粘贴',
                    ...
                    click() {
                        Editor.Ipc.sendToPanel('story-master.plot', 'onPlotMenuItemPast', data);
                    }
                },
                {type: 'separator'},
        ...
        'app:play-on-device'() {
            ...
            if (platform === PkgPreview.run) {// 作品预览
                this._saveStartScene("StoryStart")
                this._openBrowser();
            } else if (platform === PkgPreview.unit) {// 单元测试
                this._saveInitCfgUnit(function () {
                    this._saveStartScene("StoryTest");
                    this._openBrowser();
                }.bind(this));
            } else if (platform === PkgPreview.story) {// 故事预览
                this._saveStartScene("StoryGame");
                this._openBrowser();
            }
        },
